...
object SunfishEngineConfig {
    // ... Ostatak ovog objekta je nepromenjen ...
...
class SunfishEngine {
    ...
    fun setPositionFromFen(fen: String) {
        Log.d(TAG, "setPositionFromFen called with FEN: $fen")
        val parts = fen.split(" ")
    ...
    fun searchBestMove(timeLimitSec: Double = 1.0): String? {
        ...
        Log.d(TAG, "Search finished. Best move object: $bestMove with score: $score")
        ...
        if (bestMove != null) {
            ...
            // Koristimo promenljivu stanja koja je postavljena u setPositionFromFen
            if (isEngineCalculatingForBlack) {
                from = 119 - from
                to = 119 - to
            }
            // ISPRAVKA: UKLONJENA LINIJA KOJA MENJA isEngineCalculatingForBlack
            // Stanje se postavlja samo jednom, prilikom učitavanja FEN-a.

            val moveStr = "${render(from)}${render(to)}${bestMove.prom?.lowercase() ?: ""}"
            Log.d(TAG, "Returning move: $moveStr")
            return moveStr
        } else {
            Log.e(TAG, "Engine returned null move!")
            return null
        }
    }
